...
const sanitizePagination = (
  ...
  const sanitizedLimit = Math.min(
    ...
    Math.max(
      1,
      parseInt(String(limit || config.defaultPageLimit), 10) ||
  ...
  try {
    ...
    if (submissionCount > 0) {
      sendError(
        res,
        400,
        `Cannot delete problem with ${submissionCount} existing submissions`,
      )
      return
    }
...
export const getAllProblems = async (
  ...
  try {
    ...
    const sortField = allowedSortFields.includes(getStringParam(sortBy))
      ? getStringParam(sortBy)
      : 'createdAt'
    ...
    const [problems, total] = await Promise.all([
      Problem.find(query)
        .select(
          'title difficulty topics companyTags pattern submissionsCount acceptedCount slug premium createdAt',
        )
        .sort(sort)
        .skip(skip)
        .limit(sanitizedLimit)
    ...
    const problemsWithStatus: ProblemWithStatus[] = problems.map((problem) => ({
      ...
      acceptanceRate:
        problem.submissionsCount > 0
          ? ((problem.acceptedCount / problem.submissionsCount) * 100).toFixed(
              1,
            )
          : '0.0',
    }))
  ...
  try {
    ...
    const [submissions, total] = await Promise.all([
      Submission.find({ user: req.user._id, problem: id })
        .select(
          'status language runtime memory testCasesPassed testCasesTotal createdAt',
        )
        .sort({ createdAt: -1 })
        .skip(skip)
...
export const getProblemsByCompany = async (
  ...
  try {
    ...
    const problems = await Problem.find(query)
      .select(
        'title difficulty topics companyTags submissionsCount acceptedCount slug',
      )
      .lean()
 ...
 * @desc    Get problems by category (array, string, math, etc) from JSON files
 * @access  Protected
 */
export const getProblemsByCategory = async (
  ...
  try {
    ...
    // Validate category
    const validCategories = ['array', 'string', 'math']
    ...
    try {
      // Load problems from JSON file from the backend working directory.
      // This works both in dev (ts-node) and prod (node dist/app.js) because
      // `process.cwd()` remains the backend folder when started from there.
      ...
      const fileContents = await readFile(problemsPath, 'utf8')
      ...
      if (!Array.isArray(parsed)) {
        sendError(
          res,
          500,
          `Invalid problems format for category: ${categoryLower}`,
        )
        return
      }

      const allProblems = parsed

      // Parse pagination
      const pageNum = Math.max(1, parseInt(String(pageParam)) || 1)
      const limitNum = Math.min(
        100,
        Math.max(1, parseInt(String(limitParam)) || 20),
      )
      const skip = (pageNum - 1) * limitNum

      // Paginate
      const paginatedProblems = allProblems.slice(skip, skip + limitNum)
      ...
      console.error(
        `Failed to load problems for category ${categoryLower}:`,
        fileError,
      )
      sendError(
        res,
        404,
        `Problems file not found for category: ${categoryLower}`,
      )
    }
  } catch (error) {
    handleError(error, res, 'getProblemsByCategory')
  }
}
